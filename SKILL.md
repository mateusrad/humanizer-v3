...
Before scanning for other patterns, scan for `—` and `–`. Any hit means the
draft is not done.
...
Fix the specific words and phrases that contain tells. Do not restructure.
Deliver the edited text with a brief note of what changed.

### Moderate mode (medium artificialness)
1. Identify the passages with tells.
2. Rewrite those passages. Leave the rest unchanged.
3. Check for em dashes (`—`, `–`). Remove any that remain.
...
4. Write a **final rewrite** that resolves them. Confirm it contains no em or
   en dashes.
5. Deliver: draft, audit bullets, final rewrite, summary of changes.
